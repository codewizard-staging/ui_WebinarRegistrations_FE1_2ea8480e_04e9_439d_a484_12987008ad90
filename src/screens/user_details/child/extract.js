import UserJsonConfig from "config/user_product_details_detail_config.json";
...
const MapItems = [
    { navpropname: "", uicomponent: "user", expand: "", exclude: [], func: Support.AddOrUpdateUser }
]

const FetchUserInfo = async () => {
    let item = {};
    return new Promise(async (resolve) => {
        const keyItems = Object.keys(UserJsonConfig);
        keyItems.forEach(elm => {
            let items = [];
            for (let prop of UserJsonConfig[elm]) {
...
const FetchUserDetails = async (userId, enums) => {
    ...
    return new Promise(async (resolve) => {
        ...
        if (userId) {
            ...
            if($expand) {
                rslt = await Api.GetUserSingle(userId, `$expand=${$expand}`);
            }else {
                rslt = await Api.GetUserSingle(userId, null);
            }
            if (rslt.status) {

                const user = rslt.values;
                ...
                for (let i = 0; i < MapItems.length; i++) {
                    ...
                    const sourceObj = Helper.IsNullValue(source) ? user : user[source];
...
const Extract = async (userId) => {

    return new Promise(async (resolve) => {

        let rtnObj = { row: {}, options: [], backRow: {} };

        await FetchUserInfo().then(async (item) => {
            ...
            await FetchDropdownItems(items).then(async (enums) => {
                ...
                if (!Helper.IsNullValue(userId)) {
                    await FetchUserDetails(userId, enums).then(({ row, backRow }) => {
