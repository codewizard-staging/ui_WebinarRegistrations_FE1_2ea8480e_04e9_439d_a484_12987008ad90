...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, userId;
        ...
        // Add Or Update User
        rslt = await Support.AddOrUpdateUser(user, dropDownOptions, []);
        if (rslt.status) {
            userId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the User and navigation entity if it is deos not exist
            let navItem = user.find(x => x.uicomponent === mapItems[i].uicomponent);
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    data = [
                        { key: "UserId", value: parseInt(userId) },
                        { key: navItem.key, value: parseInt(rslt.id) }
                    ];
                    rslt = await Support.AddOrUpdateUser(data, dropDownOptions);
        ...
        global.AlertPopup("success", "User is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
