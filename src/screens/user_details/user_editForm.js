...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], userId, user;

        user = row['user'];
        userId = row['user'].find((x) => x.type === 'keyid').value;
            ...
            if (changes.length > 0) {
                ...
                if (tmp.length > 0) {
                    ...
                    if (rslt.status) {
                        ...
                        if (Helper.IsNullValue(mapItem.navpropname)) userId = rslt.id;
        ...
        global.AlertPopup("success", "User is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
